...
namespace LTA_PGS_20
{
    class PrintDataGrid
    {
            ...
            private static string print_test = "";
            public static void Print_dataGridView_preview(DataGridView dgv1,string print_str,string title)
            {
                PrintTitle = title;
                print_test = print_str;
            ...
            public static void Print_DataGridView(DataGridView dgv1, string print_str, string title)
            {
                PrintTitle = title;
                print_test = print_str;
                try
                {
                    ...
                    System.Windows.Forms.PrintDialog ppvw = new PrintDialog();//创建一个PrintDialog的实例。
                    ppvw.AllowSomePages = true;
                ...
                try
                {
                    ...
                    if (PageNo == 1)
                    {
                        foreach (DataGridViewColumn GridCol in dgv.Columns)
                        {
                            ...
                            //if (!PrintDataGrid.SelectedColumns.Contains(GridCol.HeaderText)) continue;
                    ...
                    while (RowPos <= dgv.Rows.Count - 1)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Draw Header
                                Font _font = new Font(dgv.Font.FontFamily, 10, FontStyle.Bold);

                                float left = e.MarginBounds.Left + ((int)(e.MarginBounds.Width / 2) -
                                        (int)(e.Graphics.MeasureString(PrintTitle, _font
                                        , e.MarginBounds.Width).Width / 2));

                                float top =  e.MarginBounds.Top -
                                        e.Graphics.MeasureString(PrintTitle, _font
                                        , e.MarginBounds.Width).Height - 26;

                                e.Graphics.DrawString(PrintTitle, _font, Brushes.Black, left, top);

                                // Draw Text
                                left = e.MarginBounds.Left;
                                top= e.MarginBounds.Top -
                                        e.Graphics.MeasureString(print_test, new Font(dgv.Font,
                                        FontStyle.Bold), e.MarginBounds.Width).Height - 2;

                                e.Graphics.DrawString(print_test, new Font(dgv.Font, FontStyle.Bold),
                                       Brushes.Black, left, top);

                                // Draw Print time
                                String s = "Print Time:" + DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss");
                                left = e.MarginBounds.Left + (e.MarginBounds.Width -
                                ...
                                top =  e.MarginBounds.Top -
                                        ...
                                        FontStyle.Bold), FontStyle.Bold), e.MarginBounds.Width).Height - 2;
                            ...
                            foreach (DataGridViewCell Cel in GridRow.Cells)
                            {
                                ...
                                {
                                    string _tem = (Cel.Value != null ? Cel.Value.ToString() : "");
                                    e.Graphics.DrawString(_tem, Cel.InheritedStyle.Font,
